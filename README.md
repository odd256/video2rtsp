...
### 1. 使用 GPU 进行推流时，运行一段时间后自动崩溃？
如果你的配置中启用了硬件加速（GPU）转码（例如使用 `h264_nvenc`），但程序在运行一段时间后异常退出，通常有以下两个原因：
- **推流数量超过 GPU 上限**：消费级显卡（如 NVIDIA GeForce 系列）通常有并发编码流数量限制（通常为 3-5 路，视具体型号和驱动而定）。如果超过限制，FFmpeg 进程会崩溃。
- **GPU 驱动与 FFmpeg 版本不匹配**：硬件加速对驱动版本有一定要求。请尝试更新显卡驱动到最新版本，并确保 FFmpeg 也是较新版本。
